...
int main()
{
    ...
    ScoringQueue queue;
    AnswerMutator mutator;

    solution(data, creator);

    constexpr int kIterationsAmount = 1 << 10;

    for (auto i = 0; i < kIterationsAmount && !queue.IsStagnating(); ++i)
    {
        mutator.MutateAnswer(creator);

        ll score = judge(data, creator);

        std::cout << score << std::endl;

        queue.Push(score);

        // if (score < queue.GetLast())
        // {
        //     mutator.Unmutate(creator);
        //     queue.PushSame();
        // }
        // else
        // {
        //     queue.Push(judge(data, creator));
        // }
    }
